...
`宇宙学原理`的“通常意义”是，在足够大尺度上，宇宙在某一“宇宙学时间”切片上对“基本观测者”（fundamental observers）的空间分布是`统计均匀（statistical homogeneity）`且`统计各向同性（statistical isotropy）`的；等价地（在适当技术条件下），宇宙的大尺度时空几何可用 **FLRW（Friedmann–Lemaître–Robertson–Walker）** 类度量作为背景来建模。
...
**它的本质是一条“对称性/平均化的建模公设”**：CP不是从 GR 方程逻辑必然推出的定理，而是把不可承受的自由度（一般时空）压缩为极少数全局参数（$$a(t),k,\Lambda,\rho(t),p(t)$$等）的“结构性假设”，其合理性来自观测与（可能的）动力学机制（如暴胀的各向同性化）。
...
我们直接观测的是过去光锥；CP却断言的是空间切片上的统计性质。把“光锥上近似各向同性”外推到“空间上近似均匀各向同性”，必须额外引入**哥白尼原理（Copernican principle）**或同类“我们不是特殊位置”的典型性假设。

`哥白尼原理（Copernican）`的核心断言是：我们（地球/银河系/本地观测者）在宇宙中不处于“特权位置”或“特权方向”；在现代形式中常被表述为：任意“基本观测者”（fundamental/comoving observers）的观测统计性质在大尺度上应无系统差别。它本质上是一个认识论—建模层面的典型性（typicality）假设，用来把“我们在一处看到的现象”推广为“宇宙整体的性质”。
  - `弱哥白尼原理（spatial non-privilege）`：我们不在宇宙的几何中心、特殊点或特殊轴上；宇宙不存在“以我们为中心”的特选结构（至少在可观测尺度上）。
  - `强哥白尼原理（statistical typicality across observers）`：所有基本观测者在足够大尺度上应看到同分布的统计性质（例如大尺度各向同性/同质性统计量一致）。这常与“平庸原理（principle of mediocrity）”相近：我们是某个观测者集合中的“典型样本”。




1. **证据结构**：CMB 的近各向同性与其统计检验总体支持 FLRW/$$\Lambda$$CDM，同时也确认了一些大角尺度“异常”（anomalies）。Planck 2018 的系统统计分析显示整体与 $$\Lambda$$CDM 高度一致但保留异常张力。([aanda.org][1])
