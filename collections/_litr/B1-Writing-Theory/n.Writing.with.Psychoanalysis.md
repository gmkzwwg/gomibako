...
```mermaid
flowchart LR
    I[Imaginary\nimage identification\nvoice surface coherence]
    S[Symbolic\nlaw language kinship\ngenre plot institution]
    R[Real\ntrauma bodily excess\nunsymbolizable intrusion]

    I --> C[Character mask and self-image]
    S --> P[Plot constraint and social code]
    R --> B[Break in narration or scene reality]

    C --> D[Desire and rivalry]
    P --> D
    B --> D

    D --> M[Motif repetition and displaced symptom]
```
...
```mermaid
flowchart TD
    A[Mirror or identification trigger]
    B[Lack becomes conscious as desire]
    C[Objet petit a cue appears]
    D[Symbolization attempts explanation]
    E[Explanation slips or fails]
    F[Real intrusion\nbody shock silence glitch break]
    G[Narrative displacement\nmotif joke dream detour repetition]
...
```mermaid
flowchart LR
A[Temperament and bodily need] --> B[Holding and co-regulation]
B --> C[Attachment expectations]
C --> D[Procedural and implicit relational memory]
D --> E[Play and transitional symbols]
E --> F[Childhood narrative scaffolding]
F --> G[Adult transference and repetition]
G --> H[Revision through new experience]
```
...
```mermaid
flowchart TD
A[Early attachment and holding] --> B[Attachment expectations]
B --> C[Transference]
C --> D[Repetition]
E[Interoception and body state] --> F[Affect]
F --> G[Defense]
G --> H[Action]
H --> I[Post-hoc explanation]
I --> J[Narrative identity]
K[Trauma cues] --> E
K --> D
J --> D
```
