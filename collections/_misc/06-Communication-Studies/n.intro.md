...
```mermaid
graph TD
    A[Communication Studies] --> B(Rhetoric)
    A --> C(Interpersonal)
    A --> D(Mass/Media)
    A --> E(Organizational)
    B --> F(Aristotle: Ethos/Pathos/Logos)
    B --> G(Modern Rhetoric, Burke)
    C --> H(Symbolic Interaction)
    C --> I(Social Penetration Theory)
    D --> J(Media Effects Theories)
    D --> K(Cultural Studies)
    D --> L(Techno-Social Networks)
    E --> M(Organizational Culture)
    E --> N(Leadership/Teams)
    J --> O(Agenda-Setting)
    J --> P(Cultivation)
    J --> Q(Spiral of Silence)
    J --> R(Diffusion of Innovations)
    K --> S(Encoding/Decoding)
    K --> T(Critical Theory)
    L --> U(Social Network Analysis)
    U --> V(Information Diffusion)
    V --> R
```
...
```mermaid
timeline
    title Major Milestones in Communication Studies
    335BC  : Aristotle - Rhetoric (persuasion by ethos/pathos/logos)
    1927   : Lasswell (propaganda analysis; 5-Ws of communication)
    1940   : Lazarsfeld (Two-Step Flow: media via opinion leaders)
    1948   : Shannon (Information Theory; channels, noise)
    1954   : Schramm (mass communication textbook)
    1964   : McLuhan (Medium is the Message; media ecology)
    1972   : McCombs & Shaw (Agenda-Setting)
    1974   : Katz & Blumler (Uses and Gratifications)
    1974   : Noelle-Neumann (Spiral of Silence)
    1976   : Gerbner (Cultivation Theory of TV)
    1984   : Habermas (Theory of Communicative Action)
    1990s  : Internet & Social Media Emergence
    2000s  : Big Data Analytics, Network Analysis
```
