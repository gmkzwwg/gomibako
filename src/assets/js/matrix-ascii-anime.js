 ...
 *   - Characters are grouped by visual role and animation swaps only within each group.
...
(function () {
  ...
  var CONFIG = {
    ...
    charGroups: {
      ...
      upper: "=-.,",                              // upper-edge group
      lower: "=-`'"                              // lower-edge group
    },
    ...
    frameMs: 150,                                  // animation frame interval
    waveSpeed: 0.0045,                             // time phase
    waveX: 0.9,                                    // x phase
    waveY: 0.55,                                   // y phase
  ...
  /**
   * Clone config with nested charGroups.
  ...
  function cloneConfig(src) {
    ...
    out.charGroups = {};
    for (key in src.charGroups) {
      if (Object.prototype.hasOwnProperty.call(src.charGroups, key)) {
        out.charGroups[key] = src.charGroups[key];
      }
    }
  ...
  function mergeConfig(base, next) {
    ...
    for (key in next) {
      if (Object.prototype.hasOwnProperty.call(next, key) && key !== "charGroups") {
        out[key] = next[key];
      }
    }

    if (next.charGroups) {
      for (key in next.charGroups) {
        if (Object.prototype.hasOwnProperty.call(next.charGroups, key)) {
          out.charGroups[key] = next.charGroups[key]; // group-level merge
        }
      }
    }
  ...
  /**
   * Convert alpha matrix into visual-role cells.
  ...
  /**
   * Pick a character within one fixed visual group.
   * @param {string} group - Character group name.
   * @param {number} phase - Animated phase value.
   * @returns {string}
   */
  function pickGroupedChar(group, phase) {
    var chars = currentConfig.charGroups[group] || "";
    var idx;

    if (!chars.length) return " ";

    idx = Math.abs(Math.floor(phase * chars.length)) % chars.length; // group-local swap
  ...
  function buildFrame(state, now) {
    ...
    var t = now * currentConfig.waveSpeed;
    var lines = [];
    var y;
    var x;
    var line;
    ...
    for (y = 0; y < rows; y++) {
      ...
      for (x = 0; x < cols; x++) {
        ...
        wave = Math.sin(t + x * currentConfig.waveX + y * currentConfig.waveY + state.seed);
        phase = wave + cell.alpha * 3.17 + x * 0.11 + y * 0.07; // stable variation
        line += pickGroupedChar(cell.group, phase); // same-group replacement only
      }
